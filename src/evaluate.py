...
def plot_predictions(y_true, predictions, feature_names, n_features):
    ...
    for feature_idx, feature_name in enumerate(feature_names):
        ...
        ax.plot(y_true[x_beg:x_end, 0, feature_idx], 
                label='Actual', color='black', linewidth=2)
        
        colors = ['blue', 'green', 'red', 'orange', 'purple']
        for (name, pred), color in zip(predictions.items(), colors):
            ax.plot(pred[x_beg:x_end, 0, feature_idx], 
                    label=name, linestyle='--', alpha=0.7, color=color)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(plot_dir, f'prediction_{feature_name.replace(" ", "_")}.png'))
        plt.close()
...
def plot_mae_comparison(results):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bars = ax.bar(model_names, mae_values, color=['blue', 'green', 'red', 'orange', 'purple'])
...
def main():
    ...
    # Evaluate
    results = evaluate_models(data, ensemble, save_plots=True)
