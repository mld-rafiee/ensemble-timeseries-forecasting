...
from tensorflow.keras.layers import (
    Dense, SimpleRNN, LSTM, GRU, Reshape,
...
def create_model(model_type, n_steps, n_features, n_future_steps):
    ...
    if model_type == 'Transformer':
        ...
        if model_type == 'RNN':
            model.add(SimpleRNN(units=64, input_shape=(n_steps, n_features), activation='relu'))
        elif model_type == 'LSTM':
            model.add(LSTM(units=64, input_shape=(n_steps, n_features), activation='relu'))
        elif model_type == 'GRU':
            model.add(GRU(units=64, input_shape=(n_steps, n_features), activation='relu'))
        else:
