...
from metadata import load_metadata, save_metadata, should_retrain, get_epochs_done
...
def train_base_models(data, config, model_dir, initial_epoch=0):
    ...
    if initial_epoch >= total_epochs:
        print(f"✅ Models already trained for {initial_epoch} epochs (target: {total_epochs}). Skipping base model training.")
    ...
    for model_type in model_types:
        ...
        model, _ = load_or_create_model(model_type, n_steps, n_features, n_future_steps, model_dir)
...
def main():
    ...
    if not retrain_needed:
        ...
        print("   To retrain from scratch, delete the 'models' folder or change config.")
        return
    
    if reason == 'config_changed':
        print("🔄 Config changed (features, window size, etc.). Retraining from scratch.")
        initial_epoch = 0
    else:
        # Load metadata to get previous epochs
        metadata = load_metadata(model_dir)
        initial_epoch = metadata.get('epochs', 0) if metadata else 0
        print(f"📊 Previous training: {initial_epoch} epochs done. Will continue to {config['training']['epochs']}.")
